...
import { useDynamicSlice, useDynamicSliceActions } from '@pitboxdev/dynamic-store-redux';
...
export default function UserStats() {
  ...
  const { data: score, setData: setUser } = useDynamicSlice('user', userConfig, (u) => u.score);
  
  // Get actions without subscribing to 'ui' state changes (prevents UserStats re-renders when sidebar toggles)
  const { setData: setUI } = useDynamicSliceActions('ui');
    ...
    <div className="card">
      ...
      <div style={{ marginTop: '25px', paddingTop: '15px', borderTop: '1px solid #eee' }}>
         <p style={{ fontSize: '12px', color: '#7f8c8d', marginBottom: '10px' }}>
           * This component uses <code>useDynamicSliceActions</code> to 
